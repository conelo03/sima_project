    ...
    <div class="container mb-5">
       ...
       <div class="row bg-white mt-4 pt-3 p-1 rounded-3 shadow-sm">
          <div class="d-flex justify-content-between">
            <div>
                <a href="/user">< Kembali ke halaman User</a>
            </div>
            <div>
            </div>
          </div>
        ...
        <form method="post" action="/user/{{ $user->username }}" enctype="multipart/form-data">
          ...
          <div class="mb-5">
            ...
            <input type="number" class="form-control @error('angkatan') is-invalid @enderror" id="angkatan" name="angkatan" value="{{ old('angkatan', $user->angkatan) }}">
            @error('angkatan')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
          ...
          <div class="mb-3">
            ...
            <img src="{{ asset('/storage/' . $user->image) }}" alt="Foto Profil {{ $user->name }}"
                class="img-preview img-fluid my-3 d-block" width="100">
        @else
            <img class="img-preview img-fluid my-3" width="100">
        @endif
          ...
          <div class="mb-3">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" name="status" id="status1" value="Aktif" @checked(old('status', $user->status))>
              <label class="form-check-label" for="status">
                Aktif
               </label>
             </div>
             <div class="form-check">
               <input class="form-check-input" type="radio" name="status" id="status2" value="Non Aktif" @checked(old('status', $user->status))>
               <label class="form-check-label" for="status">
                 Non Aktif
               </label>
             </div>
